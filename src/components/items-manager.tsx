...
export function ItemsManager({ type, title, description }: { type: "product" | "service"; title: string; description: string }) {
  const [items, setItems] = useState<Item[]>([]);
  const [categories, setCategories] = useState<Category[]>([]);
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Item | null>(null);
  const [form, setForm] = useState({
    name: "", description: "", sku: "", category_id: "", price: "0", cost: "0",
    tax_rate: "0", currency: "USD", unit: type === "service" ? "hour" : "unit",
    track_inventory: type === "product", stock_quantity: "0", is_active: true,
  });

  async function load() {
    const [i, c] = await Promise.all([
      supabase.from("items").select("*, item_categories(name, color)").eq("type", type).order("created_at", { ascending: false }),
  ...
  function openEdit(it: Item) {
    setEditing(it);
    setForm({
      name: it.name, description: it.description ?? "", sku: it.sku ?? "",
      category_id: it.category_id ?? "", price: String(it.price), cost: String(it.cost),
      tax_rate: String(it.tax_rate), currency: it.currency, unit: it.unit ?? "unit",
      track_inventory: it.track_inventory, stock_quantity: String(it.stock_quantity),
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openNew} className="bg-gradient-hero"><Plus className="h-4 w-4" /> New {title.toLowerCase().replace(/s$/, "")}</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg">
            ...
            <form onSubmit={save} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>{type === "product" ? "SKU" : "Code"}</Label><Input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} /></div>
                <div>
                  <Label>Category</Label>
                  <Select value={form.category_id} onValueChange={(v) => setForm({ ...form, category_id: v })}>
                    <SelectTrigger><SelectValue placeholder="None" /></SelectTrigger>
                    <SelectContent>
                      {categories.length === 0 && <div className="px-2 py-1.5 text-sm text-muted-foreground">No categories yet</div>}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div><Label>Tax %</Label><Input type="number" step="0.01" value={form.tax_rate} onChange={(e) => setForm({ ...form, tax_rate: e.target.value })} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              {type === "product" && form.track_inventory && (
                <div><Label>Stock quantity</Label><Input type="number" step="1" value={form.stock_quantity} onChange={(e) => setForm({ ...form, stock_quantity: e.target.value })} /></div>
              )}
              <div className="flex items-center justify-between rounded-md border p-3">
                <Label>Active</Label>
      ...
      <div className="bg-card border rounded-xl">
        {items.length === 0 ? (
          <div className="p-12 text-center text-muted-foreground">No {title.toLowerCase()} yet.</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {items.map((it) => (
                <TableRow key={it.id}>
                  ...
                  {type === "product" && <TableCell className="text-right">{it.track_inventory ? Number(it.stock_quantity) : "—"}</TableCell>}
                  <TableCell>{!it.is_active && <Badge variant="secondary">Inactive</Badge>}</TableCell>
