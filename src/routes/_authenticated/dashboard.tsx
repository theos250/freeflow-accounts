...
import {
  DollarSign, Receipt, TrendingUp, FileText, Wallet, AlertTriangle,
  ArrowUpRight, ArrowDownRight, Plus, UserPlus, FileSpreadsheet, Briefcase, Users,
} from "lucide-react";
import {
...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      const [i, e, c, b] = await Promise.all([
        ...
        (supabase as any).from("bank_accounts").select("id,current_balance,currency,is_active").eq("is_active", true),
      ]);
  ...
  const deltas = useMemo(() => {
    ...
    return {
      revenue: { pct: pct(thisMonth.revenue, lastMonth.revenue), up: thisMonth.revenue >= lastMonth.revenue },
      expenses: { pct: pct(thisMonth.expenses, lastMonth.expenses), up: thisMonth.expenses <= lastMonth.expenses },
  ...
  const kpis = [
    { label: "Revenue", value: stats.revenue, icon: DollarSign, delta: deltaLabel(deltas.revenue), up: deltas.revenue?.up ?? true, tone: "text-emerald-600", bg: "bg-emerald-500/10" },
    { label: "Expenses", value: stats.expenses, icon: Receipt, delta: deltaLabel(deltas.expenses), up: deltas.expenses?.up ?? true, tone: "text-amber-600", bg: "bg-amber-500/10" },
    { label: "Profit", value: stats.profit, icon: TrendingUp, delta: deltaLabel(deltas.profit), up: deltas.profit?.up ?? true, tone: "text-primary", bg: "bg-primary/10" },
    { label: "Cash balance", value: stats.cash, icon: Wallet, delta: "", up: true, tone: "text-violet-600", bg: "bg-violet-500/10" },
    { label: "Outstanding", value: stats.outstanding, icon: FileText, delta: "", up: true, tone: "text-blue-600", bg: "bg-blue-500/10" },
    { label: "Overdue bills", value: stats.overdue, icon: AlertTriangle, delta: "", up: false, tone: "text-rose-600", bg: "bg-rose-500/10" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl border bg-card p-4 hover:shadow-md transition-shadow">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border bg-card">
          ...
          <div className="divide-y">
            {invoices.length === 0 && <div className="p-6 text-sm text-muted-foreground text-center">No invoices yet. <Link to="/invoices" className="text-primary hover:underline">Create one →</Link></div>}
            {invoices.slice(0, 5).map((i) => (
              <div key={i.id} className="flex items-center justify-between p-4">
                <div>
                  <div className="font-medium text-sm">{i.invoice_number}</div>
                  <div className="text-xs text-muted-foreground">{i.customer_id ? customerMap.get(i.customer_id) ?? "—" : "No customer"} · {i.issue_date}</div>
                </div>
                <div className="text-right">
                  <div className="font-semibold text-sm">{fmt(Number(i.total))}</div>
                  <div className={`text-xs uppercase tracking-wide ${i.status === "paid" ? "text-emerald-600" : i.status === "overdue" ? "text-rose-600" : "text-muted-foreground"}`}>{i.status}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border bg-card">
          ...
          <div className="divide-y">
            {expenses.length === 0 && <div className="p-6 text-sm text-muted-foreground text-center">No expenses yet. <Link to="/expenses" className="text-primary hover:underline">Scan a receipt →</Link></div>}
            {expenses.slice(0, 5).map((e) => (
              <div key={e.id} className="flex items-center justify-between p-4">
                <div>
                  <div className="font-medium text-sm">{e.vendor ?? "Unknown vendor"}</div>
                  <div className="text-xs text-muted-foreground">{e.category ?? "Uncategorized"} · {e.expense_date}</div>
                </div>
