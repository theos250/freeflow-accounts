...
type Row = { account_id: string; code: string; name: string; type: string; debit: number; credit: number };
const DEBIT_NORMAL = new Set(["asset", "expense"]);

function TrialBalancePage() {
  const currency = useDefaultCurrency();
  const fmt = (n: number) => formatCurrency(n, currency);
  const [from, setFrom] = useState(() => { const d = new Date(); d.setMonth(0, 1); return d.toISOString().slice(0, 10); });
  ...
  useEffect(() => {
    (async () => {
      ...
      const accounts = (accRes.data ?? []) as { id: string; code: string; name: string; type: string }[];
      const lines = (lineRes.data ?? []) as unknown as { account_id: string; debit: number; credit: number }[];
      ...
      const built: Row[] = accounts.map((a) => {
        ...
        return { account_id: a.id, code: a.code, name: a.name, type: a.type, debit: d + d2, credit: c + c2 };
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-6">Debit/credit totals per account for the selected period.</p>
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && <tr><td colSpan={5} className="px-3 py-6 text-center text-muted-foreground">No activity in this period.</td></tr>}
            {rows.map((r) => (
          ...
          {rows.length > 0 && (
            <tfoot className="bg-muted/30">
              ...
              <tr>
                <td colSpan={5} className={"px-3 py-2 text-right text-sm " + (Math.round(totals.d * 100) === Math.round(totals.c * 100) ? "text-green-600" : "text-destructive")}>
                  {Math.round(totals.d * 100) === Math.round(totals.c * 100) ? "In balance" : `Out of balance by ${fmt(Math.abs(totals.d - totals.c))}`}
                </td>
              </tr>
            </tfoot>
          )}
