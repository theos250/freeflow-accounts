...
type Line = {
  ...
  journal_entries: { entry_date: string; reference: string | null; memo: string | null; source_type: string | null } | null;
};
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("journal_lines")
        .select("id,debit,credit,description,journal_entries!inner(entry_date,reference,memo,source_type)")
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && <tr><td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">No transactions in this period.</td></tr>}
            {rows.map((r) => (
              <tr key={r.id} className="border-t">
                <td className="px-3 py-2">{r.journal_entries?.entry_date}</td>
                <td className="px-3 py-2">{r.journal_entries?.reference}{r.journal_entries?.source_type && r.journal_entries.source_type !== "manual" && <span className="ml-2 text-xs px-1.5 py-0.5 rounded bg-muted">{r.journal_entries.source_type}</span>}</td>
                <td className="px-3 py-2 text-muted-foreground">{r.description ?? r.journal_entries?.memo}</td>
                <td className="px-3 py-2 text-right font-mono">{Number(r.debit) ? fmt(Number(r.debit)) : ""}</td>
                <td className="px-3 py-2 text-right font-mono">{Number(r.credit) ? fmt(Number(r.credit)) : ""}</td>
          ...
          {rows.length > 0 && (
            <tfoot className="bg-muted/30">
              <tr>
                ...
                <td className="px-3 py-2 text-right font-mono">{fmt(rows[rows.length - 1].balance)}</td>
              </tr>
            </tfoot>
          )}
        </table>
