...
type Estimate = {
  id: string; estimate_number: string; customer_id: string | null; issue_date: string; expiry_date: string | null;
  status: string; subtotal: number; tax: number; total: number; currency: string; converted_invoice_id: string | null;
...
function EstimatesPage() {
  ...
  const [form, setForm] = useState({
    estimate_number: "", customer_id: "", issue_date: new Date().toISOString().slice(0, 10),
    expiry_date: "", status: "draft", currency: defaultCurrency, notes: "",
  });
  const [lines, setLines] = useState<Line[]>([]);

  async function load() {
    const [est, cust, cat] = await Promise.all([
      (supabase as any).from("estimates").select("*, customers(name)").order("issue_date", { ascending: false }),
      supabase.from("customers").select("id,name").order("name"),
      supabase.from("items").select("id,name,price,tax_rate").eq("is_active", true).order("name"),
    ]);
    if (est.error) toast.error(est.error.message); else setItems(est.data as Estimate[]);
  ...
  const totals = useMemo(() => {
    ...
    for (const l of lines) {
      ...
      subtotal += a; tax += a * ((Number(l.tax_rate) || 0) / 100);
    }
  ...
  async function openEdit(e: Estimate) {
    setEditing(e);
    setForm({
      estimate_number: e.estimate_number, customer_id: e.customer_id ?? "",
      issue_date: e.issue_date, expiry_date: e.expiry_date ?? "", status: e.status,
      currency: e.currency, notes: "",
    });
    const { data } = await (supabase as any).from("estimate_items")
      .select("item_id,description,quantity,unit_price,tax_rate").eq("estimate_id", e.id);
  ...
  const addBlank = () => setLines((l) => [...l, { item_id: null, description: "", quantity: 1, unit_price: 0, tax_rate: 0 }]);
  const updateLine = (i: number, p: Partial<Line>) => setLines((l) => l.map((ln, idx) => idx === i ? { ...ln, ...p } : ln));
  ...
  async function save(e: React.FormEvent) {
    ...
    if (!u.user) return;
    const payload: any = {
      user_id: u.user.id, estimate_number: form.estimate_number,
      customer_id: form.customer_id || null, issue_date: form.issue_date,
      expiry_date: form.expiry_date || null, status: form.status, currency: form.currency,
      subtotal: totals.subtotal, tax: totals.tax, total: totals.total, notes: form.notes,
    };
    let estId = editing?.id;
    if (editing) {
      const { error } = await (supabase as any).from("estimates").update(payload).eq("id", editing.id);
      if (error) return toast.error(error.message);
    } else {
      const { data, error } = await (supabase as any).from("estimates").insert(payload).select("id").single();
    ...
    if (estId) {
      ...
      if (lines.length > 0) {
        const rows = lines.map((l) => ({
          estimate_id: estId, user_id: u.user!.id, item_id: l.item_id,
          description: l.description || "Item", quantity: Number(l.quantity) || 0,
  ...
  async function convertToInvoice(est: Estimate) {
    if (est.converted_invoice_id) { toast.info("Already converted"); return; }
    const { data: u } = await supabase.auth.getUser(); if (!u.user) return;
    const { data: estLines } = await (supabase as any).from("estimate_items")
      .select("item_id,description,quantity,unit_price,tax_rate,amount").eq("estimate_id", est.id);
    const invPayload = {
      user_id: u.user.id, invoice_number: `INV-${Date.now().toString().slice(-6)}`,
      customer_id: est.customer_id, issue_date: new Date().toISOString().slice(0, 10),
      due_date: null, status: "draft", currency: est.currency,
      subtotal: est.subtotal, tax: est.tax, total: est.total,
    ...
    if (estLines && estLines.length > 0) {
      const rows = estLines.map((l: any) => ({
        invoice_id: inv.id, user_id: u.user!.id, item_id: l.item_id,
    ...
    await (supabase as any).from("estimates").update({ status: "converted", converted_invoice_id: inv.id }).eq("id", est.id);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Send quotes and proposals; convert them to invoices in one click.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openNew} className="bg-gradient-hero"><Plus className="h-4 w-4" /> New estimate</Button>
          </DialogTrigger>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editing ? "Edit estimate" : "New estimate"}</DialogTitle></DialogHeader>
            <form onSubmit={save} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Estimate #</Label><Input required value={form.estimate_number} onChange={(e) => setForm({ ...form, estimate_number: e.target.value })} /></div>
                <div>
              ...
              <div><Label>Currency</Label><CurrencySelect value={form.currency} onValueChange={(v) => setForm({ ...form, currency: v })} /></div>
              ...
              <div className="border rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Select value="" onValueChange={addCatalog}>
                      ...
                      <SelectContent>
                        {catalog.length === 0 && <div className="px-2 py-1.5 text-sm text-muted-foreground">No items yet</div>}
                ...
                {lines.length === 0 ? (
                  <div className="text-sm text-muted-foreground text-center py-6">Add items to get started.</div>
                ) : (
              ...
              <Button type="submit" className="w-full bg-gradient-hero">Save estimate</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="bg-card border rounded-xl">
        {items.length === 0 ? (
          ...
          <Table>
            <TableHeader><TableRow><TableHead>#</TableHead><TableHead>Customer</TableHead><TableHead>Date</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Total</TableHead><TableHead className="w-40"></TableHead></TableRow></TableHeader>
            <TableBody>
              {items.map((e) => (
                <TableRow key={e.id}>
                  ...
                  <TableCell><Badge variant={e.status === "converted" ? "default" : "secondary"}>{e.status}</Badge></TableCell>
                  <TableCell className="text-right">{fmt(Number(e.total), e.currency)}</TableCell>
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => convertToInvoice(e)} title="Convert to invoice" disabled={!!e.converted_invoice_id}><FileCheck className="h-4 w-4" /></Button>
