...
const emptyForm = { txn_date: new Date().toISOString().slice(0, 10), description: "", amount: "", category_account_id: "", reference: "" };
...
function TransactionsPage() {
  ...
  async function loadAccounts() {
    const [a, g] = await Promise.all([
      (supabase as any).from("bank_accounts").select("id,name,currency").eq("is_active", true).order("name"),
      (supabase as any).from("accounts").select("id,code,name,type").eq("is_active", true).in("type", ["expense", "revenue"]).order("code"),
    ]);
    if (a.data) {
  ...
  async function loadTxns() {
    ...
    const { data, error } = await (supabase as any)
      ...
      .limit(500);
    if (error) toast.error(error.message); else setTxns(data as Txn[]);
    setLoading(false);
  }
  useEffect(() => { loadTxns(); /* eslint-disable-next-line */ }, [accountId]);
  ...
  async function setCategory(id: string, categoryId: string) {
    const { error } = await (supabase as any).from("bank_transactions").update({ category_account_id: categoryId || null }).eq("id", id);
    if (error) return toast.error(error.message);
    setTxns((prev) => prev.map((t) => (t.id === id ? { ...t, category_account_id: categoryId || null } : t)));
  }
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = {
      user_id: u.user.id, bank_account_id: accountId, txn_date: form.txn_date,
  ...
  async function remove(t: Txn) {
    if (t.is_transfer) return toast.error("Delete this from the Transfers page instead — it will remove both sides");
    ...
    <div>
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground">Search, categorize, and manage every transaction in an account.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openNew} className="bg-gradient-hero" disabled={!accountId}><Plus className="h-4 w-4" /> New transaction</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg">
            ...
            <form onSubmit={save} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><Label>Amount</Label><Input required type="number" step="0.01" placeholder="-50.00 or 250.00" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></div>
              </div>
              <p className="text-xs text-muted-foreground -mt-2">Positive = money in, negative = money out.</p>
              <div>
                <Label>Category (optional)</Label>
                <Select value={form.category_account_id} onValueChange={(v) => setForm({ ...form, category_account_id: v })}>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <Button variant={uncategorizedOnly ? "default" : "outline"} size="sm" onClick={() => setUncategorizedOnly((v) => !v)}>
          Uncategorized only
        </Button>
      </div>
                ...
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    {t.is_transfer ? (
                      ...
                      <Select value={t.category_account_id ?? ""} onValueChange={(v) => setCategory(t.id, v)}>
                        <SelectTrigger className="w-52 h-8"><SelectValue placeholder="Uncategorized" /></SelectTrigger>
                        <SelectContent>
                          {glAccounts.map((g) => <SelectItem key={g.id} value={g.id}>{g.code} {g.name}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )}
                  </TableCell>
                  <TableCell className={`text-right tabular-nums font-medium ${Number(t.amount) < 0 ? "text-red-600" : "text-emerald-600"}`}>
                    {fmt(Number(t.amount), currency)}
                  </TableCell>
                  <TableCell>{t.reconciled && <Badge variant="outline">Reconciled</Badge>}</TableCell>
