...
type Transfer = {
  id: string; from_account_id: string; to_account_id: string; amount: number;
...
function TransfersPage() {
  ...
  async function load() {
    ...
    const [a, t] = await Promise.all([
      (supabase as any).from("bank_accounts").select("id,name,currency,current_balance").eq("is_active", true).order("name"),
      (supabase as any).from("bank_transfers")
        .select("*, from:bank_accounts!from_account_id(name), to:bank_accounts!to_account_id(name)")
        .order("transfer_date", { ascending: false }).order("created_at", { ascending: false }).limit(200),
    ]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await (supabase as any).rpc("create_bank_transfer", {
      _from_account_id: fromId, _to_account_id: toId, _amount: amt,
      _transfer_date: date, _notes: notes.trim() || null,
    });
    setSaving(false);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Move money between your own accounts and keep both balances accurate.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-card border rounded-xl p-4">
            <h3 className="font-semibold mb-3 flex items-center gap-2"><ArrowLeftRight className="h-4 w-4" /> New transfer</h3>
            <form onSubmit={submit} className="space-y-3">
              <div>
                ...
                <Select value={fromId} onValueChange={setFromId}>
                  ...
                  <SelectContent>
                    {accounts.map((a) => <SelectItem key={a.id} value={a.id}>{a.name} ({fmt(Number(a.current_balance), a.currency)})</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <Select value={toId} onValueChange={setToId}>
                  ...
                  <SelectContent>
                    {accounts.filter((a) => a.id !== fromId).map((a) => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Amount</Label><Input required type="number" step="0.01" min="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} /></div>
                <div><Label>Date</Label><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></div>
              </div>
              {fromAccount && <p className="text-xs text-muted-foreground">Available in {fromAccount.name}: {fmt(Number(fromAccount.current_balance), fromAccount.currency)}</p>}
              <div><Label>Notes (optional)</Label><Textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} /></div>
              <Button type="submit" className="w-full bg-gradient-hero" disabled={saving}>{saving ? "Saving…" : "Record transfer"}</Button>
            </form>
          </div>
        </div>
          ...
          <div className="bg-card border rounded-xl">
            {loading ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Date</TableHead><TableHead>From</TableHead><TableHead></TableHead><TableHead>To</TableHead>
                    <TableHead className="text-right">Amount</TableHead><TableHead>Notes</TableHead><TableHead className="w-12"></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {transfers.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right tabular-nums font-medium">{fmt(Number(t.amount))}</TableCell>
                      <TableCell className="text-sm text-muted-foreground">{t.notes ?? "—"}</TableCell>
                      <TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => remove(t)}><Trash2 className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
