...
function BankFeedsPage() {
  ...
  async function load() {
    const { data } = await (supabase as any).from("bank_accounts").select("id,name,currency").eq("is_active", true).order("name");
  ...
  function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      const descIdx = header.findIndex((h) => h.includes("desc") || h.includes("memo") || h.includes("narration"));
      ...
      for (const line of raw.slice(1)) {
        ...
        if (Number.isNaN(amount)) { bad++; continue; }
  ...
  async function confirmImport() {
    ...
    const payload = rows.map((r) => ({
      user_id: u.user!.id, bank_account_id: accountId, txn_date: r.txn_date,
      description: r.description, amount: r.amount, import_batch: batch,
    }));
    ...
    for (let i = 0; i < payload.length; i += chunkSize) {
      ...
      if (error) { setImporting(false); return toast.error(error.message); }
    }
    setImporting(false);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Import transactions from a CSV export of your bank or mobile money statement.</p>
      </div>
      ...
      <div className="bg-card border rounded-xl p-6 mb-6 space-y-4">
        ...
        <Alert>
          ...
          <AlertDescription>
            A header row with <strong>Date</strong> and either <strong>Amount</strong> (positive = in, negative = out)
            or separate <strong>Debit</strong>/<strong>Credit</strong> columns. A <strong>Description</strong> column is optional but recommended.
          </AlertDescription>
        </Alert>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="bg-card border rounded-xl p-4">
              ...
              {skipped > 0 && <div className="text-xs text-muted-foreground mt-1">{skipped} row{skipped > 1 ? "s" : ""} skipped (unreadable date/amount)</div>}
            </div>
            <div className="bg-card border rounded-xl p-4">
              ...
              <TableBody>
                {rows.slice(0, 300).map((r, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell className={`text-right tabular-nums ${r.amount < 0 ? "text-red-600" : "text-emerald-600"}`}>{fmt(r.amount, currency)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            {rows.length > 300 && <div className="p-3 text-center text-xs text-muted-foreground">Showing first 300 of {rows.length} rows</div>}
          </div>

          <Button onClick={confirmImport} disabled={importing || !accountId} className="bg-gradient-hero">
            <CheckCircle2 className="h-4 w-4" /> {importing ? "Importing…" : `Import ${rows.length} transactions`}
          </Button>
        </>
      )}
    </div>
