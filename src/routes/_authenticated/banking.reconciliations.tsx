...
type BankAccount = { id: string; name: string; currency: string; current_balance: number; opening_balance: number };
type Txn = {
  id: string; txn_date: string; description: string; amount: number; reconciled: boolean; is_transfer: boolean;
};
...
function ReconciliationsPage() {
  ...
  async function loadAccounts() {
    const { data } = await (supabase as any).from("bank_accounts").select("id,name,currency,current_balance,opening_balance").eq("is_active", true).order("name");
    if (data) {
  ...
  const reconciledTotal = useMemo(() => reconciled.reduce((s, t) => s + Number(t.amount), 0), [reconciled]);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Match your books to a bank statement, one transaction at a time.</p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input type="number" step="0.01" className="w-40" value={statementBalance} onChange={(e) => setStatementBalance(e.target.value)} placeholder="0.00" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        ...
        <Card><CardContent className="pt-6">
          ...
          <div className="text-xs text-muted-foreground mt-1">{fmt(unreconciled.reduce((s, t) => s + Number(t.amount), 0), currency)}</div>
        </CardContent></Card>
        <Card><CardContent className="pt-6">
          <div className="text-sm text-muted-foreground">Difference vs statement</div>
          <div className={`text-xl font-bold ${difference !== null && Math.abs(difference) > 0.005 ? "text-red-600" : "text-emerald-600"}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <Button variant={showUnreconciledOnly ? "default" : "outline"} size="sm" onClick={() => setShowUnreconciledOnly((v) => !v)}>
          {showUnreconciledOnly ? "Showing unmatched only" : "Showing all transactions"}
        </Button>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => markAllVisible(true)}>Mark all visible reconciled</Button>
          <Button variant="outline" size="sm" onClick={() => markAllVisible(false)}>Mark all visible unreconciled</Button>
        </div>
      </div>
      ...
      <div className="bg-card border rounded-xl">
        {loading ? (
          ...
          <div className="p-12 text-center text-muted-foreground">{showUnreconciledOnly ? "Everything is reconciled." : "No transactions yet."}</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {visible.map((t) => (
                <TableRow key={t.id}>
                  <TableCell><Checkbox checked={t.reconciled} onCheckedChange={(v) => toggle(t, v === true)} /></TableCell>
                  <TableCell className="text-sm">{t.txn_date}</TableCell>
                  <TableCell className="text-sm">{t.description || "—"} {t.is_transfer && <Badge variant="secondary" className="ml-1">Transfer</Badge>}</TableCell>
