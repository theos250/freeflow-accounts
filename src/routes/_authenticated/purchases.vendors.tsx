...
type Vendor = {
  ...
  payment_terms: number; tax_id: string | null; is_1099: boolean; notes: string | null;
};

const empty = { name: "", email: "", phone: "", address: "", payment_terms: 30, tax_id: "", is_1099: false, notes: "" };
...
function VendorsPage() {
  ...
  async function load() {
    ...
    if (error) toast.error(error.message); else setItems(data as Vendor[]);
  }
  useEffect(() => { load(); }, []);
  ...
  function openEdit(v: Vendor) {
    ...
    setForm({
      ...
      payment_terms: v.payment_terms, tax_id: v.tax_id ?? "", is_1099: v.is_1099, notes: v.notes ?? "",
    });
    setOpen(true);
  }
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = {
      ...
      payment_terms: Number(form.payment_terms) || 0, tax_id: form.tax_id || null,
      is_1099: form.is_1099, notes: form.notes || null,
    };
    const q = editing
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Suppliers, contact details, payment terms, and 1099 status.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button onClick={openNew} className="bg-gradient-hero"><Plus className="h-4 w-4" /> New vendor</Button></DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={save} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Payment terms (days)</Label><Input type="number" value={form.payment_terms} onChange={(e) => setForm({ ...form, payment_terms: Number(e.target.value) })} /></div>
                <div><Label>Tax ID / EIN</Label><Input value={form.tax_id} onChange={(e) => setForm({ ...form, tax_id: e.target.value })} /></div>
              </div>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={form.is_1099} onCheckedChange={(v) => setForm({ ...form, is_1099: !!v })} />
                Requires 1099 reporting
              </label>
              <div><Label>Notes</Label><Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
              <Button type="submit" className="w-full bg-gradient-hero">{editing ? "Save changes" : "Create vendor"}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
        ...
        {items.length === 0 ? (
          ...
          <Table>
            <TableHeader><TableRow>
              ...
              <TableHead>Terms</TableHead><TableHead>Tax ID</TableHead><TableHead>1099</TableHead>
